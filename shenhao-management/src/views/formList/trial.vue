...
export default {
  ...
  methods: {
    ...
    handleOk(e) {
      ...
      dealTrial({ pmCode: this.selectPmCode, dealResult: this.dealResult }).then((res) => {
        if (!res.success) {
    ...
    handleCommentsList(current = 1, pageSize = 10) {
      ...
      getTrialList({ phone, isDeal, dealResult, startTime, endTime, current, pageSize }).then((res) => {
        this.loading = false
        if (res.success) {
          const { data } = res
          // 数据转化
